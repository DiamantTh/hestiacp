	...
	>
		...
		<div class="form-container">
			...
			<div id="password-details" class="u-mb20">
				...
				<div x-cloak x-show="!loginDisabled" id="passkey-options" class="u-mt10">
					<p class="form-label u-text-bold"><?= tohtml(_("Passkeys (WebAuthn / FIDO2)")) ?></p>
					<div id="passkey-error-msg" class="error" style="display:none;"></div>
					<div id="passkey-success-msg" class="u-mb10" style="display:none;color:green;"></div>
					<?php if (!empty($v_passkeys)) { ?>
						<ul id="passkey-list" class="u-mb10" style="list-style:none;padding:0;">
							<?php foreach ($v_passkeys as $pk_id => $pk_data) { ?>
								<li class="u-side-by-side u-mb5" data-pk-id="<?= tohtml($pk_id) ?>">
									<span>
										<i class="fas fa-key u-mr5"></i>
										<?= tohtml($pk_data["name"]) ?>
										<small class="u-text-muted"> &mdash; <?= tohtml($pk_data["created"]) ?></small>
									</span>
									<button
										type="button"
										class="button button-secondary button-small js-delete-passkey"
										data-id="<?= tohtml($pk_id) ?>"
										data-token="<?= tohtml($_SESSION["token"]) ?>"
									>
					...
					<?php } else { ?>
						<p id="passkey-none-msg" class="u-mb10"><?= tohtml(_("No passkeys registered.")) ?></p>
					<?php } ?>
					<?php if (function_exists("openssl_verify")) { ?>
						<div class="u-side-by-side u-mb10">
							<input type="text" id="passkey-name" class="form-control" placeholder="<?= tohtml(_("Passkey name (e.g. My YubiKey)")) ?>" maxlength="64">
							<button type="button" id="passkey-register-btn" class="button button-secondary">
								<i class="fas fa-plus"></i><?= tohtml(_("Add Passkey")) ?>
							</button>
						</div>
					<?php } ?>
				</div>
...
(function () {
	...
	function showError(msg) {
		var el = document.getElementById("passkey-error-msg");
		if (el) { el.textContent = msg; el.style.display = "block"; }
		var sel = document.getElementById("passkey-success-msg");
		if (sel) sel.style.display = "none";
	}
	function showSuccess(msg) {
		var el = document.getElementById("passkey-success-msg");
		if (el) { el.textContent = msg; el.style.display = "block"; }
		var eel = document.getElementById("passkey-error-msg");
	...
	if (registerBtn) {
		registerBtn.addEventListener("click", function () {
			...
			if (!name) {
				showError(<?= json_encode(_("Please enter a name for the passkey.")) ?>);
				return;
			}
			var token = <?= json_encode($_SESSION["token"]) ?>;
			fetch("/passkey/register/?token=" + encodeURIComponent(token))
				.then(function (r) { return r.json(); })
				.then(function (options) {
					if (options.error) { showError(options.error); return Promise.reject(options.error); }
					options.challenge = b64ToBuffer(options.challenge);
					options.user.id = b64ToBuffer(options.user.id);
					if (options.excludeCredentials) {
						options.excludeCredentials = options.excludeCredentials.map(function (c) {
							c.id = b64ToBuffer(c.id); return c;
						});
					}
				...
				.then(function (result) {
					if (!result) return;
					if (result.success) {
						showSuccess(<?= json_encode(_("Passkey registered successfully. Reload the page to see it listed.")) ?>);
						if (nameInput) nameInput.value = "";
						var noneMsg = document.getElementById("passkey-none-msg");
				...
				.catch(function (err) {
					if (err && err.name !== "NotAllowedError") {
						showError(<?= json_encode(_("Passkey registration failed. Please try again.")) ?>);
					}
				});
		});
	}
	...
	document.addEventListener("click", function (e) {
		var btn = e.target.closest(".js-delete-passkey");
		if (!btn) return;
		if (!confirm(<?= json_encode(_("Are you sure you want to remove this passkey?")) ?>)) return;
		var credId = btn.dataset.id;
		var token = btn.dataset.token;
		fetch("/passkey/delete/", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ token: token, credential_id: credId })
		})
			.then(function (r) { return r.json(); })
			.then(function (result) {
				if (result.success) {
					...
					if (li) li.remove();
					showSuccess(<?= json_encode(_("Passkey removed.")) ?>);
				} else {
					showError(result.error || <?= json_encode(_("Failed to remove passkey.")) ?>);
				}
			})
			.catch(function () {
				showError(<?= json_encode(_("Failed to remove passkey.")) ?>);
			});
	});
})();
</script>
